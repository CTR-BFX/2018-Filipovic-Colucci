...
__Note__: Differential gene expression analysis was performed with DESeq2 (v1.16.0), before the introduction of `lfcShrink` in version 1.18.0. If the latest version of DESeq2 is used to recreate this analysis we have included extra code in the R script to apply `lfcShrink` if the version is >= 1.18.0. However as the shrinkage methods are different the results will not be identical.
